...
def compute_heads_importance(
    args, model, eval_dataloader, compute_entropy=True, compute_importance=True, head_mask=None, verbose=True, device_id=0
):
    ...
    n_layers = model.mnetwork.bert.config.num_hidden_layers
    n_heads = model.mnetwork.bert.config.num_attention_heads
    ...
    # Print/save matrices (since we will run multiple trials, let's turn this off for now)
    ...
    return attn_entropy, head_importance, head_ranks, preds, labels
...
def main():
    ...
    if args.do_improper:
      dev_data._data = np.array(dev_data._data)[data[~data['proper']]['idx'].to_numpy()].tolist()
    dev_data_loader = DataLoader(dev_data, batch_size=args.batch_size, collate_fn=collater.collate_fn, pin_memory=opt['cuda'])
